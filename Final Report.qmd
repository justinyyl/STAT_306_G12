...
To clarify whether including the interaction terms would significantly improve our model's predictive performance, we conducted a likelihood ratio test based on Wilks’ theorem to compare the additive model against a model with interaction terms. \par

We defined the hypothesis as follows:

-   Null hypothesis $H_0$: logistic regression of additive model is true model

-   Alternative hypothesis $H_a$: logistic regression of interaction model is true model \par
...
Table 3 presents the inference results from the logistic regression model.
